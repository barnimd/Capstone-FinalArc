...
public class EvaluationPanel_Tp4 : MonoBehaviour
{
    [Header("=== Root Panel ===")]
    ...
    public TMP_Text txtQuestionNum;
    ...
    [Tooltip("Label topik di header, mis. 'RANSOMWARE & BACKUP'. Kosongkan kalau teksnya sudah diisi manual.")]
    ...
    [Tooltip("Label teks pilihan. Kalau diisi, teks TIDAK diberi prefix 'A. ' / 'B. ' (sudah ada badge).")]
    public TMP_Text labelChoiceA;
    public TMP_Text labelChoiceB;
    ...
    private EvaluationData_Tp4 _data;
    ...
    public void MulaiEvaluasi(EvaluationData_Tp4 data)
    {
        _data = data;
    ...
    private void TampilkanSoal(int index)
    {
        ...
        EvaluationQuestion_Tp4 soal = _data.questions[index];

        if (txtQuestionNum != null)
            txtQuestionNum.text = $"Pertanyaan {index + 1} dari {_data.questions.Length}";
        if (txtQuestionText != null)
            txtQuestionText.text = soal.questionText;

        SetChoiceLabel(labelChoiceA, btnChoiceA, "A", soal.choiceA);
        SetChoiceLabel(labelChoiceB, btnChoiceB, "B", soal.choiceB);

        var nextLabel = btnNext.GetComponentInChildren<TMP_Text>();
        if (nextLabel != null)
            nextLabel.text = (index == _data.questions.Length - 1) ? "Selesai" : "Selanjutnya";
    ...
    private void SetChoiceLabel(TMP_Text explicitLabel, Button button, string prefix, string text)
    {
        if (explicitLabel != null)
        {
            // Desain baru: huruf A/B sudah tampil di badge, jadi teksnya polos.
            explicitLabel.text = text;
            return;
        }

        var label = button != null ? button.GetComponentInChildren<TMP_Text>() : null;
        if (label != null) label.text = $"{prefix}. {text}";
    }
    ...
    private void UpdateButtonColors()
    {
        for (int i = 0; i < 2; i++)
        {
            ...
            TMP_Text label = i == 0 ? labelChoiceA : labelChoiceB;
            if (label != null) label.color = selected ? choiceTextSelectedColor : choiceTextDefaultColor;
        }
    }
    ...
    private void BangunBarisHasil()
    {
        ...
        for (int i = 0; i < total; i++)
        {
            EvaluationQuestion_Tp4 soal = _data.questions[i];
    ...
    private void OnSelesaiClicked()
    {
        ...
        if (EvaluationManager_Tp4.Instance != null)
            EvaluationManager_Tp4.Instance.SelesaiEvaluasi();
    }
